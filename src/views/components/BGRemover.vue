<template>
    <form class="mb-2 relative">
        ...
        <div v-if="message.text" class="absolute top-0 left-0 rounded-lg bg-gray-200 opacity-70 w-full h-full "></div>
    </form>
    <div v-if="message.text" div class="flex flex-col mt-4 p-8 bg-white border-2 border-dashed border-red-400 rounded-md">
  ...
  export default {
    ...
    methods: {
      ...
      onFileChange: async function(file = null) {
        ...
        if ((this.user && this.user.credits > 0) || (!this.user && !this.getSessionCode())) {
          this.isLoading = true
          ...
          this.message.text = "Your 1 credits trial ended, create your account now and win 10 credits more !"
